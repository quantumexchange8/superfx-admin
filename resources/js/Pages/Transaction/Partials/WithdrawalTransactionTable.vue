...
<template>
    ...
    >
        ...
        <template v-if="transactions?.length > 0">
            ...
            >
                <template #header>
                    <span class="hidden md:block text-nowrap">{{ $t('public.requested_on') }}</span>
                </template>
                <template #body="{data}">
            ...
            >
                ...
                <template #body="{data}">
                    <span v-if="data.from_login" class="break-all">{{ data?.from_meta_login }}</span>
                    <span v-else-if="data.from_wallet">{{ $t(`public.${data?.from_wallet.type}`) }}</span>
                    <span v-else>-</span>
                </template>
            </Column>
            <Column
            ...
            >
                ...
                <template #body="{data}">
                    <div v-if="data.from_login" class="flex gap-2 items-center">
                        <Tag
            ...
            >
                ...
                <template #body="{data}">
                    <span class="break-all">{{ formatAmount(data?.amount ?? 0) }}</span>
                </template>
            </Column>
            <Column
                field="approved_at"
            ...
            >
                <template #header>
                    <span class="hidden md:block text-nowrap">{{ $t('public.approval_on') }}</span>
                </template>
                <template #body="{data}">
                    <div v-if="['successful', 'rejected'].includes(data.status)">
                        {{ dayjs(data.approved_at).format('YYYY/MM/DD') }}
                        <div class="text-xs text-gray-500">
                            {{ dayjs(data.approved_at).format('HH:mm:ss') }}
                        </div>
                    </div>
                    <div v-else>
                        -
                    ...
                    <div class="flex items-center gap-2 self-stretch">
                        <div class="flex flex-col items-start w-full">
                            ...
                            <div class="flex gap-1 items-center text-xs">
                                ...
                                <span>|</span>
                                <div class="text-gray-700 font-medium"> {{ data.from_login ? data.from_meta_login : (data.from_wallet ? $t(`public.${data.from_wallet.type}`) : '-') }}</div>
                            </div>
                        </div>
                        ...
                        <div
                            :class="[
                                'font-semibold',
                                {
                                    ...
                                    'text-gray-950': data.status === 'cancelled',
                                }
                            ]"
                        >
    ...
    <OverlayPanel ref="op">
        <div class="flex flex-col gap-5 w-60 py-5 px-4">
            ...
            <div class="flex flex-col gap-2 items-center self-stretch">
                ...
                <div class="flex flex-col self-stretch">
                    <div
                        v-for="status in ['successful', 'failed', 'processing', 'cancelled']"
                        class="flex items-center gap-2 text-sm text-gray-950"
                    >
                        <div class="flex justify-center items-center rounded-full grow-0 shrink-0 hover:bg-gray-100">
                            <RadioButton
    ...
    >
        <div class="flex flex-col justify-center items-start pb-4 gap-3 self-stretch border-b border-gray-200 md:flex-row md:pt-4 md:justify-between">
            <!-- below md -->
            <span class="md:hidden self-stretch text-gray-950 text-xl font-semibold">${{ formatAmount(data.amount ?? 0) }}</span>
            ...
            <!-- above md -->
            <span class="hidden md:block w-[180px] text-gray-950 text-right text-xl font-semibold">${{ formatAmount(data.amount ?? 0) }}</span>
        </div>
        ...
        <div class="flex flex-col items-center py-4 gap-3 self-stretch border-b border-gray-200">
            ...
            <div class="flex flex-col md:flex-row items-start gap-1 self-stretch">
                <span class="self-stretch md:w-[140px] text-gray-500 text-xs">{{ data.category === 'trading_account' ? $t('public.account') : $t('public.wallet') }}</span>
                <span class="self-stretch text-gray-950 text-sm font-medium">{{ data.category === 'trading_account' ? data.from_meta_login : $t(`public.${data.from_wallet.type}`) }}</span>
            </div>
            <div v-if="data.category === 'trading_account'" class="flex flex-col md:flex-row items-start gap-1 self-stretch">
        ...
        <div class="flex flex-col items-center py-4 gap-3 self-stretch border-b border-gray-200">
            ...
            <div v-if="data.payment_platform_name"  class="flex flex-col md:flex-row items-start gap-1 self-stretch">
            ...
            <div v-if="data.to_wallet_address"  class="flex flex-col md:flex-row items-start gap-1 self-stretch">
        ...
        <div class="flex flex-col items-center py-4 gap-3 self-stretch">
            <div class="flex flex-col md:flex-row items-start gap-1 self-stretch">
                <span class="self-stretch md:min-w-[140px] text-gray-500 text-xs">{{ $t('public.remarks') }}</span>
